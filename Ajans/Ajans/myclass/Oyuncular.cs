...
{
    public class Oyuncular : Calisanlar
    {
        ...
        private string query = "SELECT * FROM oyuncu";
        ...
        {
            DT.Clear();
            string query_filter = "SELECT * FROM oyuncu WHERE tc='" + search_text + "'";
            MySqlDataAdapter DA = new MySqlDataAdapter(query_filter, connectdb);
            DA.Fill(DS);
            DT = DS.Tables[0];
        }

        public void Oyuncu_Turu(string tur)
        {
            int[] dizi = { 0, 1, 2 };
            string[] cesit = { "Sinema", "Reklam", "Radyo" };
            for(int i = 0; i < 3; i++)
            {
                if (cesit[i] == tur)
                {
                    DT.Clear();
                    string query_filter = "SELECT * FROM oyuncu WHERE player_kategori_id='" + dizi[i] + "'";
                    MySqlDataAdapter DA = new MySqlDataAdapter(query_filter, connectdb);
                    DA.Fill(DS);
                    DT = DS.Tables[0];
                }
        ...
        {
            connectdb.Open();
            string query = "INSERT INTO oyuncu(tc,name,surname,mobile,gender,marital_status,salary,player_kategori_id) VALUES (@tc,@name,@surname,@mobile,@gender,@marital_status,@salary,@player_kategori_id)";
            using (var cmd =new MySqlCommand())
            {
                cmd.CommandText = query;
                ...
                cmd.Parameters.Add("@player_kategori_id", MySqlDbType.VarChar).Value = kategori_id;
        ...
        {
            connectdb.Open();
            string query = "UPDATE oyuncu SET tc=@tc, name=@name, surname=@surname,mobile=@mobile,gender=@gender,marital_status=@marital_status, salary=@salary, player_kategori_id=@player_kategori_id WHERE tc=@fulltc";
            using (var cmd = new MySqlCommand())
            {
                cmd.CommandText = query;
                cmd.CommandType = CommandType.Text;
        ...
        {
            connectdb.Open();
            string query = "DELETE FROM oyuncu WHERE tc=@fulltc";
            using (var cmd = new MySqlCommand())
            {
                cmd.CommandText = query;
                cmd.CommandType = CommandType.Text;
